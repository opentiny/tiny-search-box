...
const renderless = (props, hooks, { emit, nextTick, refs }) => {
  ...
  const isDateRangeInvalid = computed(() => {
    const { prevItem, dateRangeFormat, datetimeRangeFormat } = props.state
    const type = prevItem.type
    if (type === 'dateRange') {
      const { startDate, endDate } = props.state
      const dateFormat = prevItem.format || dateRangeFormat
      return Boolean(startDate && endDate && compareDate(endDate, startDate, dateFormat) < 0)
    }
    if (type === 'datetimeRange') {
      const { startDateTime, endDateTime } = props.state
      const dateFormat = prevItem.format || datetimeRangeFormat
      return Boolean(startDateTime && endDateTime && compareDate(endDateTime, startDateTime, dateFormat) < 0)
    }
    return false
  })
